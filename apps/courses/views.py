...
class CourseInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request, course_id):
        ...
        all_user_comments = UserCourse.objects.filter(user_id__in=user_ids)     # 传入用户的列表，得到该列表中所有用户课程id的list
        # 取出所有课程id
        course_ids = [user_course.course.id for user_course in all_user_comments]
        # 获取学过该用户学过其他的所有课程
...
class CommentView(LoginRequiredMixin, View):
    ...
    def get(self, request, course_id):
        ...
        user_ids = [user_course.user.id for user_course in user_courses]     # 找出学了该课程的用户id，组合成一个list
        all_user_comments = UserCourse.objects.filter(user_id__in=user_ids)
        # 取出所有课程id
        course_ids = [user_course.course.id for user_course in all_user_comments]
        # 获取学过该用户学过其他的所有课程
        relate_courses = Course.objects.filter(id__in=course_ids).order_by('-click_nums')[:5]
